...
import { acquireLease,newLeaseOwner,portfolioLeaseKey,portfolioEntryLeaseKey,releaseLease,type RuntimeLease } from './leases';
...
export type RuntimeLeaseProvider={acquire(portfolioId:string,ownerId:string):Promise<RuntimeLease|null>;release(lease:RuntimeLease):Promise<unknown>;allowed?(id:string):Promise<boolean>};
...
const mongoLeases:RuntimeLeaseProvider={allowed:portfolioEntryAllowed,acquire:async(portfolioId,ownerId)=>{const gate=await acquireLease(portfolioEntryLeaseKey(portfolioId),newLeaseOwner('spawn'),30_000);if(!gate)return null;try{return await portfolioEntryAllowed(portfolioId)?await acquireLease(portfolioLeaseKey(portfolioId),ownerId,30_000):null;}finally{await releaseLease(gate);}},release:lease=>releaseLease(lease)};
...
export class TradingRuntimeManager{
  ...
  async synchronize(){
    ...
    const eligible=await Promise.all(rows.map(async row=>!this.leases.allowed||await this.leases.allowed(row._id.toHexString())));
    const configured=new Map(rows.filter((_,index)=>eligible[index]).map(row=>[row._id.toHexString(),row]));
    for(const [id,handle] of this.instances){if(!configured.has(id)){this.intentionalStops.add(id);handle.process.kill('SIGTERM');}}
