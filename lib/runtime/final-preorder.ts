...
export type FinalPreOrderReason='ENVIRONMENT_NOT_VERIFIED'|'FINAL_PREORDER_ROBOT_STOPPED'|'FINAL_PREORDER_AUTO_TRADE_OFF'|'FINAL_PREORDER_CONFIG_CHANGED'|'FINAL_PREORDER_PENDING_REPLACED'|'FINAL_PREORDER_PENDING_EXPIRED'|'FINAL_PREORDER_SIGNAL_CANDLE'|'FINAL_PREORDER_LEASE_LOST'|'FINAL_PREORDER_DELETION_IN_PROGRESS'|'FINAL_PREORDER_POSITION_NONZERO'|'FINAL_PREORDER_POSITION_INVALID'|'FINAL_PREORDER_DAILY_LOSS_BLOCKED'|'FINAL_PREORDER_MARGIN_INVALID'|'FINAL_PREORDER_RISK_GUARD'|'FINAL_PREORDER_PORTFOLIO_MISMATCH';
export type FinalPending={direction:'long'|'short';trigger:number;sl:number;candleTime:number;configRevision:string};
export type FinalConfig={revision:string;autoTrade:boolean;verified:boolean;entryValidCandles:number;resolutionSec:number;riskPct:number;rr:number;minStopPct:number;maxEffectiveLeverage:number;maxFeeRiskPct:number;gstPct:number};
...
export type FinalPreOrderDependencies={robotRunning:()=>boolean;refreshConfig:()=>Promise<FinalConfig>;currentConfig?:()=>FinalConfig;dailyLossEntryAllowed?:()=>boolean;currentPending:()=>FinalPending|null;latestCompletedCandleTime:()=>number;leaseOwned:()=>Promise<boolean>;leaseLost:()=>boolean;portfolioEntryAllowed:()=>Promise<boolean>;portfolio:()=>Promise<{id:string;environment:string;symbol:string;productId:number}|null>;position:()=>Promise<any>;availableMargin:()=>Promise<number>};
...
// Delta's product-scoped position result supports numeric sizes and decimal strings.
// Missing values, coercible booleans/arrays, and non-finite values never prove flat.
export function explicitPositionSize(position:unknown):number|null {
  if(!position||typeof position!=='object'||Array.isArray(position))return null;
  const size=(position as {size?:unknown}).size;
  if(typeof size!=='number'&&!(typeof size==='string'&&/^[+-]?(?:\d+(?:\.\d*)?|\.\d+)(?:e[+-]?\d+)?$/i.test(size.trim())))return null;
  const value=Number(size);
  if(value===0&&typeof size==='string'&&/[1-9]/.test(size.trim().split(/e/i)[0]))return null;
  return Number.isFinite(value)?value:null;
}

// No awaits: also called by the worker immediately before market dispatch.
export function finalPreOrderDispatchStateCheck(input:FinalPreOrderInput,deps:FinalPreOrderDependencies,currentConfig=deps.currentConfig?.()??input.config){
  ...
  if(currentConfig.verified!==true)return blocked('ENVIRONMENT_NOT_VERIFIED');
  ...
  if(deps.leaseLost())return blocked('FINAL_PREORDER_LEASE_LOST');
  if(deps.dailyLossEntryAllowed&&!deps.dailyLossEntryAllowed())return blocked('FINAL_PREORDER_DAILY_LOSS_BLOCKED');
...
export async function finalPreOrderSafetyCheck(input:FinalPreOrderInput,deps:FinalPreOrderDependencies){
  ...
  const initialState=finalPreOrderDispatchStateCheck(input,deps,currentConfig);
  ...
  const positionSize=explicitPositionSize(await deps.position());
  if(positionSize===null)return blocked('FINAL_PREORDER_POSITION_INVALID');
  if(positionSize!==0)return blocked('FINAL_PREORDER_POSITION_NONZERO');
  ...
  // Sizing preparation may have crossed a candle boundary or allowed control,
  // ownership, portfolio, or position state to change. Read fresh evidence after it.
  const [dispatchConfig,accountOwned,portfolioAllowed,dispatchPortfolio,dispatchPosition]=await Promise.all([
    deps.refreshConfig(),deps.leaseOwned(),deps.portfolioEntryAllowed(),deps.portfolio(),deps.position()
  ]);
  if(!dispatchConfig.autoTrade)return blocked('FINAL_PREORDER_AUTO_TRADE_OFF');
  if(dispatchConfig.verified!==true)return blocked('ENVIRONMENT_NOT_VERIFIED');
  if(dispatchConfig.revision!==input.config.revision)return blocked('FINAL_PREORDER_CONFIG_CHANGED');
  if(!accountOwned)return blocked('FINAL_PREORDER_LEASE_LOST');
  if(!portfolioAllowed)return blocked('FINAL_PREORDER_DELETION_IN_PROGRESS');
  if(!dispatchPortfolio||dispatchPortfolio.id!==input.identity.portfolioId||dispatchPortfolio.environment!==input.identity.environment||dispatchPortfolio.symbol!==input.identity.symbol||dispatchPortfolio.productId!==input.identity.productId)return blocked('FINAL_PREORDER_PORTFOLIO_MISMATCH');
  const dispatchSize=explicitPositionSize(dispatchPosition);
  if(dispatchSize===null)return blocked('FINAL_PREORDER_POSITION_INVALID');
  if(dispatchSize!==0)return blocked('FINAL_PREORDER_POSITION_NONZERO');
  // These local checks are deliberately last, after every asynchronous lookup.
  const dispatchState=finalPreOrderDispatchStateCheck(input,deps,deps.currentConfig?.()??dispatchConfig);
  if(!dispatchState.ok)return dispatchState;
