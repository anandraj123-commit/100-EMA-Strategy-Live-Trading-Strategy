...
import { RECOVERABLE_RUNTIME_EXIT_CODE,RuntimePermanentlyDeadError,TradingRuntimeManager,runtimeStartupExitCode,spawnPortfolioWorker } from './lib/runtime/manager';
import { closeMongoConnection } from './lib/db/mongodb';
import { writeSupervisorHealth } from './lib/runtime/supervision-health';
...
const portfolioId=process.env.PORTFOLIO_RUNTIME_ID?.trim()||'';
const runtimeLeaseOwner=process.env.PORTFOLIO_RUNTIME_LEASE_OWNER?.trim()||'';
let runtimeEnvironment:'real'|'demo'|null=null;
let runtimeProductId=0;
const tradeContext=()=>portfolioId&&runtimeEnvironment?{portfolioId,environment:runtimeEnvironment}:undefined;
const stableTradeId=(...args:Parameters<typeof stableTradeIdBase>)=>{const id=stableTradeIdBase(...args);return id&&runtimeEnvironment?`${runtimeEnvironment}:${portfolioId}:${id}`:id;};
const findOpenBotTrade=(productId:number,..._ignored:any[])=>findOpenBotTradeBase(productId,portfolioId);
const findOpenManualTrades=(productId:number,..._ignored:any[])=>findOpenManualTradesBase(productId,portfolioId);
const findUnresolvedBotTrades=(productId:number,..._ignored:any[])=>findUnresolvedBotTradesBase(productId,portfolioId);
const findUnresolvedManualTrades=(productId:number,..._ignored:any[])=>findUnresolvedManualTradesBase(productId,portfolioId);
const persistOpenBotTrade=(trade:any,productId:number,symbol:string,dependencies?:any,..._ignored:any[])=>persistOpenBotTradeBase(trade,productId,symbol,dependencies,tradeContext());
const persistOpenManualTrade=(trade:any,productId:number,symbol:string,fills:any[],dependencies?:any,..._ignored:any[])=>persistOpenManualTradeBase(trade,productId,symbol,fills,dependencies,tradeContext());
const persistClosedTrade=(trade:any,productId:number,symbol:string,price:number|null,dependencies?:any,..._ignored:any[])=>persistClosedTradeBase(trade,productId,symbol,price,dependencies,tradeContext());
...
let currentDay = '';
let lossStreak = 0;
let dailyLossStateReady = false;
let dailyLossStateError:string|null = 'Daily loss streak has not been restored';
const pendingDailyLossEvents=new Map<string,{outcome:BotExitOutcome;at:Date}>();
let uiLogs:any[] = [];
let tradeEvents:any[] = [];
let lastBracketSyncAt = 0;
let connectionState:'online'|'offline'|'error' = 'online';
...
function persistedSnapshot(record:any) {
  return {tradeId:record.tradeId,direction:record.side==='SHORT'?'short':'long',source:record.source,attributionStatus:record.source==='bot'?'BOT_CONFIRMED':'MANUAL_CONFIRMED',entryPrice:record.actualEntryPrice??record.intendedEntryPrice,actualEntryPrice:record.actualEntryPrice,trigger:record.intendedEntryPrice,sl:record.currentSL??record.initialSL,tp:record.currentTarget??record.takeProfit,initialSL:record.initialSL,takeProfit:record.takeProfit,protectionSlOrderId:record.protectionSlOrderId,protectionTpOrderId:record.protectionTpOrderId,contracts:record.contracts,ownedContracts:record.contracts,contractValue:record.contractValue,riskAmount:record.riskAmount??null,takerRate:record.takerRate??null,gstPct:record.gstPct??null,orderId:record.entryOrderId,clientOrderId:record.entryClientOrderId,entryFillIds:record.entryFillIds??[],entryIntentId:record.entryIntentId??null,protectionState:record.protectionState??null,strategyConfig:record.strategyConfig,openedAt:record.entryTime?.valueOf?.()??null,mixedPosition:false};
}

async function reconcileEntryIntents(){
  const result=await reconcilePortfolioEntryIntents(portfolioId);
  const recoverable=await findRecoverableConfirmedEntryIntents(portfolioId),confirmed=[...new Map([...result.confirmed,...recoverable].map(intent=>[intent.intentId,intent])).values()];
  for(const intent of confirmed){
    addTradeEvent('ENTRY_RECONCILED_CONFIRMED',{intentId:intent.intentId,clientOrderId:intent.clientOrderId,orderId:intent.deltaOrderId});
    const recovered={entrySnapshot:intent.entrySnapshot,direction:intent.direction,entryPrice:intent.actualEntryPrice??intent.trigger,actualEntryPrice:intent.actualEntryPrice,trigger:intent.trigger,sl:intent.sl,tp:intent.tp,contracts:intent.contracts,ownedContracts:intent.contracts,contractValue:intent.contractValue,orderId:intent.deltaOrderId,clientOrderId:intent.clientOrderId,entryFillIds:intent.deltaFillIds??[],entryIntentId:intent.intentId,protectionState:'PENDING',openedAt:intent.submissionStartedAt?.valueOf?.()??intent.createdAt.valueOf(),riskAmount:intent.riskAmount,takerRate:intent.takerRate,gstPct:intent.gstPct,source:'bot',attributionStatus:'BOT_CONFIRMED',mixedPosition:false,strategyConfig:intent.strategyConfig};
    await persistOpenBotTrade(recovered,intent.productId,intent.symbol);
    await markEntryIntentOwnershipPersisted(intent.intentId);
    addTradeEvent('ENTRY_CONFIRMED_PERSISTED',{intentId:intent.intentId,orderId:intent.deltaOrderId,protectionState:'PENDING'});
    lastPositionFetchAt=0;
  }
  blockingEntryIntent=await findBlockingEntryIntent(portfolioId);
  return blockingEntryIntent;
}
...
async function reconcileStaleBotTrades(lastPrice:number) {
  ...
  let unresolved:any[];
  try { unresolved=await findUnresolvedBotTrades(Number(product.id),portfolioId); }
...
async function reconcileStaleManualTrades(currentOpenTradeIds=new Set<string>()){
  ...
  let unresolved:any[];try{unresolved=await findUnresolvedManualTrades(Number(product.id),portfolioId);}catch(error:any){addTradeEvent('TRADE_HISTORY_RECONCILIATION_FAILED',{source:'exchange_existing',reason:'MONGODB_LOOKUP_FAILED',error:error?.message||String(error)});return;}
...
function dailyLossScope():DailyLossScope {
  if(!runtimeEnvironment||!runtimeProductId)throw new Error('Daily loss streak scope is not initialized');
  return {portfolioId,environment:runtimeEnvironment,productId:runtimeProductId,symbol:config.symbol};
}

async function restoreCurrentDailyLossStreak(now=new Date()){
  currentDay=tradingDayKey(now);
  try{
    const restored=await restoreDailyLossStreak(dailyLossScope(),now);
    lossStreak=restored.consecutiveLosses;
    dailyLossStateReady=true;
    dailyLossStateError=null;
    addTradeEvent('DAILY_LOSS_STREAK_RESTORED',{tradingDay:restored.tradingDay,lossStreak});
  }catch(error:any){
    dailyLossStateReady=false;
    dailyLossStateError=error?.message||String(error);
...
async function persistBotDailyLossOutcome(eventId:string,outcome:BotExitOutcome,now=new Date()){
  ...
  try{
    const result=await recordDailyLossEvent({scope:dailyLossScope(),eventId,source:'bot',attributionStatus:'BOT_CONFIRMED',outcome,at:now});
    if(result.consecutiveLosses==null)throw new Error('Daily loss event did not return durable state');
    currentDay=result.tradingDay;
    lossStreak=result.consecutiveLosses;
    dailyLossStateReady=true;
    dailyLossStateError=null;
    pendingDailyLossEvents.delete(eventId);
    addTradeEvent('DAILY_LOSS_STREAK_PERSISTED',{tradeId:eventId,outcome,tradingDay:currentDay,lossStreak,duplicate:!result.applied});
    return true;
  }catch(error:any){
    dailyLossStateReady=false;
    dailyLossStateError=error?.message||String(error);
    pendingDailyLossEvents.set(eventId,{outcome,at:now});
    addTradeEvent('DAILY_LOSS_STREAK_PERSIST_FAILED',{tradeId:eventId,outcome,error:dailyLossStateError,newEntries:'BLOCKED'});
    return false;
  }
}

async function retryPendingDailyLossEvents(){
  for(const [eventId,event] of pendingDailyLossEvents){if(!await persistBotDailyLossOutcome(eventId,event.outcome,event.at))return false;}
  return true;
}

function persistedBotOutcome(record:any):BotExitOutcome {
  if(record?.exitReason==='TP')return 'WIN';
  if(record?.exitReason==='SL')return 'LOSS';
  const gross=Number(record?.grossPnL);
  return Number.isFinite(gross)?gross>0?'WIN':gross<0?'LOSS':'BREAKEVEN':'UNKNOWN';
}
...
async function syncExchangeBracket(force = false) {
  ...
  if(!confirmed||!activeTrade.tradeId||activeTrade.mixedPosition===true||activeTrade.lifecycleReconciliationPending===true)return;
  ...
  await reconcileProtection(Number(product.id), triggerMethod, {
    ...
    triggerPrice: async (method=triggerMethod) => protectionTriggerPrice(await getTicker(config.symbol), method),
    ownership,
    load: async () => {
      const record=await findTradeLifecycle(activeTrade.tradeId,portfolioId);
      return record?.environment===runtimeEnvironment&&record.productId===Number(product.id)&&record.symbol===config.symbol?record:null;
    },
    persist: async (tradeId,values) => {
      const record=await synchronizeTradeProtection(tradeId,values,portfolioId);
      if(record&&activeTrade?.tradeId===tradeId){activeTrade.lifecycle=record;activeTrade.protectionSlOrderId=record.protectionSlOrderId;activeTrade.protectionTpOrderId=record.protectionTpOrderId;}
    },
    verifyMissing: async legs => {
      const record=await findTradeLifecycle(activeTrade.tradeId,portfolioId);
      if(!record||record.environment!==runtimeEnvironment||record.productId!==Number(product.id))return false;
      ...
      const [orders,fills]=await Promise.all([getOrderHistoryBounded(Number(product.id),start),getFillsBounded(Number(product.id),start)]);
      const proof=openExecutionPatch(record,position,fills.result,orders.result,orders.complete&&fills.complete);
      if(!proof||proof.remainingContracts!==record.remainingContracts)return false;
      const evidence=missingProtectionEvidence(record,legs,orders.result,fills.result,orders.complete&&fills.complete);
      if(!evidence.allowed)return false;
      for(const leg of evidence.terminal){await clearTerminalProtectionSubmission(record.tradeId,portfolioId,leg,record.protectionSubmissions?.[leg]?.clientOrderId??null);protectionRepairAttempts.delete(`${record.tradeId}:${leg}`);}
      return true;
    },
    claim: (legs,kind) => claimProtectionSubmission(activeTrade.tradeId,portfolioId,legs,kind,{sl:activeTrade.sl??null,tp:activeTrade.tp??null,contracts:Number(activeTrade.remainingContracts??activeTrade.ownedContracts??activeTrade.contracts),slTriggerMethod:activeTrade.lifecycle?.currentSLTriggerMethod,tpTriggerMethod:activeTrade.lifecycle?.currentTargetTriggerMethod}),
    resize: (orderId,size) => resizeProtectiveOrder(Number(product.id),orderId,size),
    bracket: (sl, tp, method=triggerMethod) => placeBracket(Number(product.id), sl, tp, method),
    stop: (side, size, leg, price, clientOrderId, method=triggerMethod) => placeProtectiveStopOrder(Number(product.id), side, size,
      leg === 'sl' ? 'stop_loss_order' : 'take_profit_order', price, method, clientOrderId),
    event: addTradeEvent,
...
async function refreshPosition(lastPrice:number, force = false) {
  ...
  if (positionSize !== 0 && !activeTrade) {
    ...
    try { [persistedBot,persistedManuals]=await Promise.all([findOpenBotTrade(Number(product.id),portfolioId),findOpenManualTrades(Number(product.id),portfolioId)]); }
    catch (error:any) { lookupFailed=true; addTradeEvent('TRADE_OWNERSHIP_LOOKUP_FAILED', { error:error?.message || String(error) }); }
    const legacyTrades=await findLegacyUnresolvedTrades(Number(product.id));
    if(legacyPositionRequiresReconciliation(positionSize,legacyTrades)){
      activeTrade={direction,entryPrice:Number(position?.entry_price||0),sl:null,tp:null,contracts:Math.abs(positionSize),ownedContracts:0,contractValue:Number(product?.contract_value||0),positionSize,orderId:null,openedAt:null,source:'unattributed',attributionStatus:'UNKNOWN',adoptedAt:Date.now(),lastObservedAt:Date.now(),mixedPosition:true,legacyTradeIds:legacyTrades.map(record=>record.tradeId),exchangeSync:null};
      pending=null;
      addTradeEvent('LEGACY_TRADE_REQUIRES_PORTFOLIO_RECONCILIATION',{productId:Number(product.id),legacyTradeIds:legacyTrades.map(record=>record.tradeId),positionSize});
    }else{
    ...
    activeTrade = ownership.status==='BOT_CONFIRMED' ? restoreOpenBotTrade(persistedBot,position,ownership,Number(product?.contract_value||0)) : ownership.status==='MANUAL_CONFIRMED' ? {
      direction,
      entryPrice:Number(position?.entry_price || 0),
      sl:restoredManual?.currentSL??restoredManual?.initialSL??null,
      lifecycle:restoredManual,initialSL:restoredManual?.initialSL,takeProfit:restoredManual?.takeProfit,
      remainingContracts:Math.abs(positionSize),
      tp:restoredManual?.currentTarget??restoredManual?.takeProfit??null,
      ...
      protectionState:restoredManual?.protectionState??null,
      exchangeSync:null
    } : {
  ...
  // Preserve the existing closed-trade streak logic for confirmed bot-created trades.
  // Manual and uncertain exchange positions remain excluded.
  if (previousPositionSize !== 0 && positionSize === 0 && activeTrade) {
    ...
    if (isBot) {
      ...
      if(!activeTrade.tradeId){dailyLossStateReady=false;dailyLossStateError='Confirmed bot close has no stable trade identity';addTradeEvent('DAILY_LOSS_STREAK_PERSIST_FAILED',{outcome:exitClassification.outcome,error:dailyLossStateError,newEntries:'BLOCKED'});}
      else await persistBotDailyLossOutcome(activeTrade.tradeId,exitClassification.outcome);
    ...
    const closedTrade = { ...activeTrade, positionClosedAt:Date.now() };
  ...
  if (positionSize !== 0 && activeTrade) {
    ...
    if(activeTrade.tradeId&&['BOT_CONFIRMED','MANUAL_CONFIRMED'].includes(activeTrade.attributionStatus)){
      try{
        let marginPosition=null;try{marginPosition=await getMarginedPosition(Number(product.id));}catch{}
        const record=await reconcileOpenTradeExecution(activeTrade,Number(product.id),position,tradeContext()!,marginPosition);
        if(record){activeTrade.lifecycleReconciliationPending=false;activeTrade.mixedPosition=false;activeTrade.lifecycle=record;activeTrade.remainingContracts=record.remainingContracts;activeTrade.actualEntryPrice=record.actualEntryPrice;activeTrade.entryPrice=record.actualEntryPrice??activeTrade.entryPrice;activeTrade.openedAt=record.entryTime?.valueOf()??activeTrade.openedAt;activeTrade.entryFillIds=record.entryFillIds;activeTrade.contracts=record.contracts;activeTrade.ownedContracts=record.contracts;}
        else {activeTrade.lifecycleReconciliationPending=true;if(Math.abs(positionSize)!==Number(activeTrade.remainingContracts??activeTrade.ownedContracts??activeTrade.contracts))activeTrade.mixedPosition=true;}
      }catch(error:any){activeTrade.lifecycleReconciliationPending=true;addTradeEvent('TRADE_LIFECYCLE_RECONCILIATION_PENDING',{tradeId:activeTrade.tradeId,error:error?.message||String(error)});}
    }
    if(activeTrade.source==='bot'&&Math.abs(positionSize)!==Number(activeTrade.remainingContracts??activeTrade.ownedContracts??0)) activeTrade.mixedPosition=true;
    activeTrade.positionSize = positionSize;
    if(activeTrade.source!=='bot'&&!activeTrade.tradeId){
...
async function cycle() {
  ...
  const bestBid=numeric(ticker.quotes?.best_bid),bestAsk=numeric(ticker.quotes?.best_ask);
  const spread=bestBid!=null&&bestAsk!=null&&bestBid>0&&bestAsk>=bestBid?
    {bid:bestBid,ask:bestAsk,amount:bestAsk-bestBid,pct:(bestAsk-bestBid)/((bestAsk+bestBid)/2)*100,observedAt:tickerObservedAt}:null;
  ...
  const strategyWasReady=strategyStateReady;
  strategyStateReady=completedCandles.length>=config.emaLen+config.slopeLookback+2;
  if(strategyStateReady&&!strategyWasReady)addTradeEvent('STRATEGY_STATE_READY',{configRevision,candleHistoryBars:completedCandles.length});
  ...
  if (candlesChanged && latest && latest.time !== lastSetupCandle) {
    ...
    if (!tradingEnabled) {
      ...
      candleDecision = { action:'WAIT', reason:pending ? 'WAITING_FOR_BREAKOUT' : 'NO_VALID_SETUP' };
    }
  ...
  if (dailyLossStateReady && strategyStateReady && tradingEnabled && !blockingEntryIntent && pending && pending.configRevision===configRevision && positionSize === 0 && lossStreak < config.maxDailyLosses && dailyLossEntryAllowed(dailyLossStateReady,lossStreak,config.maxDailyLosses) && !entryPositionOpen && pendingEntryEligible(pending,latest?.time??NaN,config.entryValidCandles,config.resolutionSec)) {
    ...
    if (breakout) {
      ...
      if(!entryLease){
        ...
        addTradeEvent('ACCOUNT_ENTRY_LOCK_BUSY',{environment:runtimeEnvironment});
      }else{
        const portfolioEntryLease=await acquireLease(portfolioEntryLeaseKey(portfolioId),newLeaseOwner(`entry:${portfolioId}`),90_000,runtimeEnvironment!);
        if(!portfolioEntryLease){await releaseLease(entryLease);decision={action:'WAIT',reason:'PORTFOLIO_DELETE_IN_PROGRESS'};addTradeEvent('PORTFOLIO_DELETE_IN_PROGRESS',{portfolioId});return;}
        try{
        await withExecutionActivity(entryLease,portfolioId,async entryExecution=>{
          if(!readControl(portfolioId).running){
      ...
      if (cachedPositionSize !== 0) {
        ...
        let configVerified=true;
        try{await refreshRuntimeSettings();}catch{configVerified=false;}
        if(!configVerified||entryConfigRevision!==configRevision||!pending||pending.configRevision!==configRevision){
          decision={action:'SKIP',reason:configVerified?'CONFIG_GENERATION_CHANGED':'CONFIG_REFRESH_FAILED'};
          if(entryConfigRevision!==configRevision)pending=null;
          addTradeEvent('ORDER_BLOCKED_CONFIG_CHANGE',{oldConfigRevision:entryConfigRevision,newConfigRevision:configRevision});
        }else if (!reject && config.autoTrade) {
          ...
          if (!readControl(portfolioId).running) {
            ...
            // Check configuration and Robot Stop once more immediately before the
            // MARKET order. Both switches must remain enabled after leverage setup.
            let finalConfigVerified=true;
            try{await refreshRuntimeSettings();}catch{finalConfigVerified=false;}
            if(!finalConfigVerified||!pending||entryConfigRevision!==configRevision||pending.configRevision!==configRevision||!config.autoTrade){
              decision={action:'SKIP',reason:!config.autoTrade?'AUTO_TRADE_FALSE':finalConfigVerified?'CONFIG_GENERATION_CHANGED':'CONFIG_REFRESH_FAILED'};
              if(entryConfigRevision!==configRevision)pending=null;
            }else if (shuttingDown||!readControl(portfolioId).running) {
              decision = { action:'STOPPED', reason:'ROBOT_STOPPED_BEFORE_ORDER' };
              if (latest) upsertUiLog(latest.time, { decision:{ action:'STOPPED', reason:'ROBOT_STOPPED_BEFORE_ORDER' } });
              addTradeEvent('ORDER_BLOCKED_ROBOT_STOPPED', { direction:pending.direction, trigger:pending.trigger });
            } else {
              ...
              const intentId=entryIntentId(identity),oid=entryClientOrderId(identity);
              const {contracts:finalContracts,riskAmount:finalRiskAmount,sl:finalSl,tp:finalTp,takerRate:finalTakerRate}=finalSafety;
              const entrySnapshot={equityAtEntry:lastWalletFetchAt>0?cachedEquity:null,...(spread&&Date.now()-Date.parse(tickerObservedAt)<=5_000?{entryBid:spread.bid,entryAsk:spread.ask,entrySpreadAmount:spread.amount,entrySpreadPct:spread.pct,entrySpreadTime:new Date(tickerObservedAt)}:{})};
              ...
              if(submission.status!=='CONFIRMED'){
                ...
                if(submission.status==='AMBIGUOUS')addTradeEvent('ENTRY_SUBMISSION_AMBIGUOUS',{intentId,clientOrderId:oid});
                return;
              }
              ...
              activeTrade = { entrySnapshot, direction:entrySetup.direction, entryPrice:Number(order?.result?.average_fill_price??entrySetup.trigger), actualEntryPrice:numeric(order?.result?.average_fill_price), trigger:entrySetup.trigger, sl:finalSl, tp:finalTp, contracts:finalContracts, ownedContracts:finalContracts, contractValue, positionSize:entrySetup.direction === 'long' ? finalContracts : -finalContracts, orderId:order?.result?.id, clientOrderId:oid, entryFillIds:Array.isArray(order?.result?.fill_ids)?order.result.fill_ids.map(String):[],entryIntentId:intentId,protectionState:'PENDING',openedAt:Date.now(), lastObservedAt:Date.now(), riskAmount:finalRiskAmount, takerRate:finalTakerRate, gstPct:config.gstPct, source:'bot', attributionStatus:'BOT_CONFIRMED', mixedPosition:false, exchangeSync:null, strategyConfig:botStrategyConfigSnapshot() };
              const persisted=await persistOpenBotTrade(activeTrade,Number(product.id),config.symbol,undefined,tradeContext());
              activeTrade.tradeId=persisted?.tradeId;activeTrade.lifecycle=persisted;
              await markEntryIntentOwnershipPersisted(intentId);
              addTradeEvent('ENTRY_CONFIRMED_PERSISTED',{intentId,orderId:order?.result?.id??null,protectionState:'PENDING'});
              addTradeEvent('PROTECTION_PENDING',{tradeId:activeTrade.tradeId,sl:finalSl,tp:finalTp});
              ...
              try{const protectionPosition=await getPosition(Number(product.id));if(explicitPositionSize(protectionPosition)===null||Number(protectionPosition.size)===0||Math.sign(Number(protectionPosition.size))!==(entrySetup.direction==='long'?1:-1))throw new Error('PROTECTION_POSITION_NOT_OPEN');await updateTradeProtectionState(activeTrade.tradeId,'PENDING');await placeBracket(Number(product.id), finalSl, finalTp, triggerMethod);bracketActive=true;activeTrade.protectionState='ACTIVE';await updateTradeProtectionState(activeTrade.tradeId,'ACTIVE');addTradeEvent('PROTECTION_ACTIVE',{tradeId:activeTrade.tradeId,sl:finalSl,tp:finalTp});}
              catch(error:any){activeTrade.protectionState='REPAIR_REQUIRED';await updateTradeProtectionState(activeTrade.tradeId,'REPAIR_REQUIRED');addTradeEvent('PROTECTION_REPAIR_REQUIRED',{tradeId:activeTrade.tradeId,error:error?.message||String(error)});}

              if (latest) {
                upsertUiLog(latest.time, {
                  order:{ market:'SENT', side, orderId:order?.result?.id ?? null, bracket:bracketActive?'SENT':'REPAIR_REQUIRED', sl:finalSl, tp:finalTp },
                  decision:{ action:'ENTRY', reason:'ALGO_POSITION_OPEN' },
                  entryEvent:true
                });
              }

              if(bracketActive)addTradeEvent('BRACKET_SENT', { orderId:order?.result?.id ?? null, sl:finalSl, tp:finalTp, triggerMethod });
        ...
        },undefined,{leaseMs:90_000,heartbeatMs:25_000,onLeaseLost:()=>addTradeEvent('ACCOUNT_ENTRY_LEASE_LOST',{environment:runtimeEnvironment})});
        }finally{await releaseLease(portfolioEntryLease);}
      }
    }
  }
  ...
  writeStatus({
    ...
    pending,
    activeTrade:activeTrade?{...activeTrade,...activeTrade.lifecycle,lifecycle:undefined,sl:activeTrade.lifecycle?.currentSL??activeTrade.sl,tp:activeTrade.lifecycle?.currentTarget??activeTrade.tp}:null,
    protection:activeTrade?.source==='bot'?{state:activeTrade.protectionState??'REPAIR_REQUIRED',status:activeTrade.protectionState==='ACTIVE'?'PROTECTED':activeTrade.protectionState==='PENDING'?'PROTECTION PENDING':'PROTECTION REPAIR REQUIRED'}:null,
    lossStreak,
    dailyLossState:{ready:dailyLossStateReady,tradingDay:currentDay,error:dailyLossStateError},
    dailyBlocked:!dailyLossStateReady||lossStreak >= config.maxDailyLosses,
    strategy:{
      ...
      entryValidCandles:config.entryValidCandles,
    ...
    tradeEvents,
    entryIntent:blockingEntryIntent?{state:blockingEntryIntent.state,status:'RECONCILING AMBIGUOUS ORDER',intentId:blockingEntryIntent.intentId,clientOrderId:blockingEntryIntent.clientOrderId}:null,
    logs:uiLogs
  },portfolioId);
}
...
async function main() {
  ...
  if(!runtimeLeaseOwner)throw new Error('Portfolio runtime lease identity is required');
  const runtimeLease={key:portfolioLeaseKey(portfolioId),ownerId:runtimeLeaseOwner};
  const requestShutdown=()=>{shuttingDown=true;};process.once('SIGTERM',requestShutdown);process.once('SIGINT',requestShutdown);
  const leaseHeartbeat=setInterval(()=>void renewLease(runtimeLease,30_000).then(ok=>{if(!ok)process.exit(RECOVERABLE_RUNTIME_EXIT_CODE);}).catch(()=>process.exit(RECOVERABLE_RUNTIME_EXIT_CODE)),10_000);
  process.once('exit',()=>clearInterval(leaseHeartbeat));
  ...
  // The manager releases the runtime lease only after observing process exit.
  await closeMongoConnection().catch(()=>{});
}

async function manage(){writeSupervisorHealth(false);const manager=new TradingRuntimeManager(listPortfolio,spawnPortfolioWorker);try{await manager.synchronize();}catch{console.error('[runtime-manager] synchronization failed');}let stopping=false;const timer=setInterval(()=>void manager.synchronize().catch(error=>{if(error instanceof RuntimePermanentlyDeadError){console.error('[runtime-manager] trading runtime repeatedly exited');writeSupervisorHealth(true);void shutdown(1);}else console.error('[runtime-manager] synchronization failed');}),10_000);const shutdown=async(exitCode=0)=>{if(stopping)return;stopping=true;shuttingDown=true;clearInterval(timer);let finalCode=exitCode;try{await manager.stopAll();}catch{console.error('[runtime-manager] portfolio worker termination failed');finalCode=1;}await closeMongoConnection().catch(()=>{});process.exit(finalCode);};process.once('SIGTERM',()=>void shutdown());process.once('SIGINT',()=>void shutdown());}
void (portfolioId?main():manage()).catch(error=>{const exitCode=error instanceof RuntimePermanentlyDeadError?1:runtimeStartupExitCode(error);console.error('[fatal]',exitCode===RECOVERABLE_RUNTIME_EXIT_CODE?'recoverable startup failure':'trading runtime cannot start');if(!portfolioId)writeSupervisorHealth(true);process.exit(exitCode);});
