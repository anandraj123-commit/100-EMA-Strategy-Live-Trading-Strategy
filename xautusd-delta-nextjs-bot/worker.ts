import { config, environmentVariables } from './lib/config';
...
import { readControl, writeStatus, readTradeHistory, writeTradeHistory } from './lib/state';
...
let uiLogs:any[] = [];
let tradeEvents:any[] = [];
let tradeHistory:any[] = readTradeHistory();
...
function addCompletedBotTrade(trade:any, exitPrice:number, exitClassification:string) {
  const direction = trade.direction === 'short' ? 'short' : 'long';
  const contracts = Math.abs(Number(trade.contracts || trade.positionSize || 0));
  const contractValue = Number(trade.contractValue || product?.contract_value || 0);
  const quantity = contracts * contractValue;
  const entryPrice = Number(trade.entryPrice || trade.trigger || 0);
  const takerRate = Number(trade.takerRate ?? product?.taker_commission_rate ?? 0);
  const gstPct = Number(trade.gstPct ?? config.gstPct);

  const entryNotional = quantity * entryPrice;
  const exitNotional = quantity * exitPrice;
  const entryBrokerage = entryNotional * takerRate;
  const exitBrokerage = exitNotional * takerRate;
  const entryGst = entryBrokerage * gstPct / 100;
  const exitGst = exitBrokerage * gstPct / 100;

  const buyBrokerage = direction === 'long' ? entryBrokerage : exitBrokerage;
  const sellBrokerage = direction === 'long' ? exitBrokerage : entryBrokerage;
  const buyGst = direction === 'long' ? entryGst : exitGst;
  const sellGst = direction === 'long' ? exitGst : entryGst;
  const brokerageTotal = buyBrokerage + sellBrokerage;
  const gstTotal = buyGst + sellGst;
  const totalCharge = brokerageTotal + gstTotal;
  const grossProfit = direction === 'long'
    ? quantity * (exitPrice - entryPrice)
    : quantity * (entryPrice - exitPrice);
  const netProfit = grossProfit - totalCharge;

  const row = {
    id:`${trade.orderId || 'trade'}-${trade.openedAt || Date.now()}`,
    orderId:trade.orderId ?? null,
    symbol:config.symbol,
    direction,
    entryTime:trade.openedAt ? new Date(trade.openedAt).toISOString() : null,
    exitTime:new Date().toISOString(),
    signalCandleTime:trade.signalCandleTime ?? null,
    entryPrice,
    sl:Number(trade.sl ?? 0),
    target:Number(trade.tp ?? 0),
    exitPrice,
    exitClassification,
    contracts,
    contractValue,
    ...
    takerRate,
    gstPct,
    riskAmount:Number(trade.riskAmount ?? 0),
    availableMarginAtEntry:Number(trade.availableMarginAtEntry ?? 0),
    riskPct:Number(trade.riskPct ?? config.riskPct),
    rr:Number(trade.rr ?? config.rr),
    chargeSource:'CALCULATED_FROM_TAKER_RATE'
  };
...
async function refreshPosition(lastPrice:number, force = false) {
  ...
  if (previousPositionSize !== 0 && positionSize === 0 && activeTrade) {
    ...
    const closeClassification = isAdopted?'EXTERNAL_POSITION_CLOSED':(won?'WIN':'LOSS_OR_MANUAL_CLOSE');
    addTradeEvent('POSITION_CLOSED', {
      direction:activeTrade.direction,
      lastPrice,
      syncedTp:tp,
      source:isAdopted?'EXISTING_DELTA_POSITION':'BOT',
      classifiedAs:closeClassification
    });
    if (!isAdopted) addCompletedBotTrade(activeTrade, lastPrice, closeClassification);
    activeTrade = null;
  }
...
async function cycle() {
  ...
  if (candlesChanged && latest && latest.time !== lastSetupCandle) {
    ...
    // A signal is executable only during the NEXT N candles configured by
    // ENTRY_VALID_CANDLES. While that window is active, a newer signal does not
    // replace the locked pending setup. Once the Nth candle has completed, the
    // old setup expires and this newly completed candle may create a fresh setup.
    if (pending && latest.time >= Number(pending.candleTime) + config.entryValidCandles * config.resolutionSec) {
      pending = null;
    }

    if (tradingEnabled && positionSize === 0 && lossStreak < config.maxDailyLosses && !pending) {
      if (s) {
        pending = {
          ...s,
          validCandles:config.entryValidCandles,
          expiresAfterCandleTime:s.candleTime + config.entryValidCandles * config.resolutionSec
        };
      }
    }
        ...
        if (!reject && config.autoTrade) {
          ...
          if (!readControl().running) {
            ...
            if (!readControl().running) {
              ...
              activeTrade = {
                direction:pending.direction,
                entryPrice:pending.trigger,
                trigger:pending.trigger,
                sl,
                tp,
                contracts,
                contractValue,
                positionSize:pending.direction === 'long' ? contracts : -contracts,
                orderId:order?.result?.id,
                openedAt:Date.now(),
                signalCandleTime:pending.candleTime,
                riskAmount,
                availableMarginAtEntry:freshAvailable,
                riskPct:config.riskPct,
                rr:config.rr,
                takerRate,
                gstPct:config.gstPct,
