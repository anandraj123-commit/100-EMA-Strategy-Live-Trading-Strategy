...
const tradeHistoryFile = path.join(dir, 'trade-history.json');
...
export function readTradeHistory(): any[] {
  try {
    const value = JSON.parse(fs.readFileSync(tradeHistoryFile, 'utf8'));
...
export function writeTradeHistory(v:any[]) {
  fs.writeFileSync(tradeHistoryFile, JSON.stringify(v, null, 2));
}
