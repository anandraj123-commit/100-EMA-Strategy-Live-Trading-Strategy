...
for(const autoTrade of [false,true])test(`real worker protection adapter repairs SL/TP when unverified, AUTO_TRADE=${autoTrade}`,async()=>{
 const {reconcileProtection,missingProtectionEvidence}=await import('../lib/trades/protection-reconciliation');const {openExecutionPatch}=await import('../lib/trades/lifecycle');const {protectionTriggerMethod,protectionTriggerPrice}=await import('../lib/trades/protection');
 ...
 vm.runInNewContext(code,{module,Date,openExecutionPatch,portfolioId:'p',runtimeEnvironment:'demo',missingProtectionEvidence,toDeltaMicroseconds:(ms:number)=>ms*1000,findTradeLifecycle:async()=>({tradeId:'open-trade',portfolioId:'p',environment:'demo',symbol:'XAUTUSD',productId:27,status:'OPEN',attributionStatus:'BOT_CONFIRMED',source:'bot',side:'LONG',entryOrderIds:['entry'],entryFillIds:['entry-fill'],contracts:2,contractValue:1,remainingContracts:2,currentSL:90,currentTarget:120,entryTime:new Date(),exitFillIds:[]}),getOrderHistoryBounded:async()=>({result:[{id:'entry',product_id:27,state:'closed'}],complete:true}),getFillsBounded:async()=>({result:[{id:'entry-fill',order_id:'entry',product_id:27,side:'buy',size:2,price:100,created_at:new Date().toISOString()}],complete:true}),claimProtectionSubmission:async()=>({sl:{clientOrderId:'sl'},tp:{clientOrderId:'tp'}}),product:{id:27},activeTrade:trade,cachedPositionSize:2,lastBracketSyncAt:0,BRACKET_SYNC_MS:5000,config:{symbol:'XAUTUSD',verified:false,autoTrade,priceSource:'mark'},protectionRepairAttempts:new Set(),protectionTriggerMethod,protectionTriggerPrice,reconcileProtection,
