...
test('MongoDB trade upsert, indexes, statistics, authentication and pagination',{skip:!uri},async()=>{
  ...
  try{
    ...
    assert.equal((await historyRoute.GET(new NextRequest(`http://localhost/api/trades?${query}&source=invalid`,{headers:{cookie}}))).status,400);assert.equal((await historyRoute.GET(new NextRequest(`http://localhost/api/trades?${query}&limit=101`,{headers:{cookie}}))).status,400);
    const page=await historyRoute.GET(new NextRequest(`http://localhost/api/trades?${query}&source=bot&limit=1`,{headers:{cookie}}));assert.equal(page.status,200);assert.equal((await page.json()).trades.length,1);assert.equal((await statsRoute.GET(new NextRequest(`http://localhost/api/trades/stats?${query}`,{headers:{cookie}}))).status,200);
